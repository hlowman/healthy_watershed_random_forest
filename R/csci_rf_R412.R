...
csci_df <- read_csv("data_raw/csci_rf_data1.csv") %>% # Loads in dataset pulled 12/3/20
  rename(COMID = comid) %>%
...
# Examine the environment to be sure # of observations looks like the 75/25 split. 1512:507.
...
nottrain <- ca %>% # all COMIDS from StreamCat data, sampled or not
  filter(!COMID %in% mydf2_train$COMID) # Removing sites used to train the model. n = 139198
...
# Mazor et al. 2016 1st percentile = 0.63
# Likely condition approach: Compare mean to three CSCI thresholds (0.63, 0.79, 0.92) @ reference sites (1st, 10th, 30th percentiles)
# Very likely altered: mean < 0.63
...
#   #mutate(classification = case_when(csci_predicted < 0.63~"Very Likely Altered", 
...
#### Results .csv ####
# Export results.
write_csv(ca_predictions2, "data_model_outputs/csci_rf_results_R412.csv")
...
write_csv(ca_sum, "data_model_outputs/csci_rf_results_summary_R412.csv")
...
(val3 <- ggplot(full_train_test, aes(x = csci_predicted, y = csci, color = set_f)) +
  ...
  theme_bw() +
  theme(legend.position = "bottom")) # n = 2,019
...
write_csv(csci_lms,"data_model_outputs/csci_lms_R412.csv" )
...
# Export process summary information
write_csv(process_summary, "data_model_outputs/csci_process_summary_R412.csv")
...
# Statewide map using 1 threshold & 2 categories
...
# png(file="L:/RipRAM_ES/Data/Working/healthy_watershed_random_forest/Results using published thresholds/csci_modeled_CA_Mazor_R4.1.2.png", units="in", width=8, height=5, res=300)
# modeled_csci_map
# dev.off()
